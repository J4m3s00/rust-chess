...
impl<'a> Lichess<'a> {
    pub fn new(game : &'a mut Game) -> Lichess<'a> {
        dotenv().ok();
        let auth = "Bearer ".to_owned() + &std::env::var("LICHESS_TOK").expect("LICHESS_TOK not set");
    ...
    pub async fn get_challenge(&self) -> Result<Challenge, String> {
        ...
        while let Some(chunk) = response.chunk().await.expect("Failed to read chunk") {
            ...
            if json.is_object() {
                ...
                if event == "challenge" {
                    let challenge: Challenge = serde_json::from_value(json.get("challenge").unwrap().clone()).unwrap();
                    // Accept challenge
                    self.client.post(BASE_URL.to_string() + "/challenge/" + &challenge.id + "/accept")
                                .header("Authorization", self.auth.clone())
                                .send()
                                .await.expect("Failed to accept challenge");
    ...
    pub async fn stream_game(&mut self, chal : Challenge) -> Result<(), ()> {
        ...
        let challenger_team = if chal.final_color == "white" { Color::White } else { Color::Black };
            ...
            if let Some(chunk) = response.chunk().await.expect("Failed to read game chunk") {
                ...
                if chunk_json.is_object() {
                    ...
                    if chunk_json.get("state").is_some() {
                        state = serde_json::from_value(chunk_json.get("state").unwrap().clone()).unwrap();
                    } else {
