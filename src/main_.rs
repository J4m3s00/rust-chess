...
impl GameState {
    ...
    pub fn make_move(&mut self, m: Move) {
        let square_to_move = self.board.get_piece(m.from);
        ...
        if let MoveType::Capture = current_move.move_type {
            if let Some(captured) = self.board.get_piece(current_move.to) {
    ...
    pub fn unmake_move(&mut self, current_move: Move) {
        let square_to_move = self.board.get_piece(current_move.to);
        if square_to_move.is_none() {
            println!("No piece to move!");
            return;
        }
        ...
        match current_move.move_type {
            MoveType::BishopPromotion | MoveType::KnightPromotion | MoveType::QueenPromotion | MoveType::RookPromotion => {
                self.board.add_piece(Piece::new(piece_to_move.color, PieceType::Pawn, current_move.from));
                self.board.remove_piece(current_move.to);
            },
            MoveType::EnPassantCapture => {
                let capture_pos = if let Team::White = piece_to_move.color { current_move.to - 8 } else { current_move.to + 8 };

                self.board.move_piece(Move::new(current_move.to, current_move.from));
                self.board.add_piece(Piece::new(piece_to_move.color.other(), PieceType::Pawn, capture_pos));
            },
            MoveType::Capture => {
                ...
                self.board.move_piece(Move::new(current_move.to, current_move.from));
                self.board.add_piece(Piece::new(piece_to_move.color.other(), capture_type, current_move.to));
            }
            _ => {
                self.board.move_piece(Move::new(current_move.to, current_move.from));
            }
    ...
    fn get_pins(&self) -> u64 {
        let mut pins : u64 = 0;
        /*let king_pos = self.board.get_king(self.turn).position;
        let king_square = self.board.squares[king_pos as usize].unwrap();
        let king_moves = king_square.get_possible_moves(self, false);
        for m in king_moves.iter() {
            if let Some(piece) = self.board.squares[m.to as usize] {
...
struct Board {
    ...
    piece_map: HashMap<Team, HashMap<PieceType, Vec<u8>>>
}
...
impl Board {
    ...
    fn add_piece(&mut self, piece: Piece) {
        assert!(piece.position < 64);
        self.squares[piece.position as usize] = Some(piece);
        
        // Add piece to piece map
        if let Some(team_map) = self.piece_map.get_mut(&piece.color) {
            if let Some(positions) = team_map.get_mut(&piece.piece_type) {
                positions.push(piece.position);
            } else {
                team_map.insert(piece.piece_type, vec![piece.position]);
            }
        } else {
            let mut team_map = HashMap::new();
            team_map.insert(piece.piece_type, vec![piece.position]);
            self.piece_map.insert(piece.color, team_map);
        }
    }

    fn remove_piece(&mut self, position: u8) {
        assert!(position < 64);
        self.squares[position as usize] = None;
    }
